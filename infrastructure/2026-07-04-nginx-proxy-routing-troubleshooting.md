...
프록시와 라우팅을 더 공부하되, 추상적인 개념을 늘리기보다 실제 트래픽 전달 구조에서 바로 쓰이는 내용 위주로 정리한다.

이 문서에서 Blue/Green은 배포 전략 자체가 아니라 Nginx가 두 upstream 중 active 대상을 선택하는 구현 사례다. 전략별 선택 기준과 trade-off는 [DevOps 배포 전략 비교](../devops/03-deployment-strategies.md)에서 다룬다.
...
장애가 나면 어떤 증상으로 보이는가?
내가 만든 Nginx 기반 트래픽 전환 구조와 어떻게 연결되는가?
```
