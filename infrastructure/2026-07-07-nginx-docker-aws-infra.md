...
```text
DNS
→ 도메인 이름을 ALB 또는 서버 주소로 연결한다.

ALB
→ HTTP/HTTPS 요청을 받고 정상 target으로 전달한다.

Security Group
→ ALB, EC2, 관리용 SSH 접근 범위를 제한한다.

Nginx
→ 서버 내부에서 active 컨테이너로 요청을 프록시한다.

Docker
→ blue/green 컨테이너를 서로 다른 host port로 노출한다.

CI/CD
→ 새 컨테이너 실행, health check, Nginx 전환, rollback을 자동화한다.

Monitoring / Logging
→ 장애가 난 지점을 찾기 위해 메트릭과 로그를 수집한다.
```

## 장애가 났을 때 연결해서 보는 방법

요청이 실패하면 아래 순서로 구간을 나눠서 보면 된다.

```text
1. DNS가 올바른 ALB 또는 서버를 가리키는가?
2. ALB listener와 target group health check가 정상인가?
3. Security Group이 ALB → EC2 요청을 허용하는가?
4. Nginx가 요청을 받고 올바른 upstream으로 넘기는가?
5. Docker host port가 active 컨테이너로 매핑되어 있는가?
6. 백엔드 애플리케이션 health check가 정상인가?
7. 로그에서 404, 502, 503, 504 중 어떤 증상이 보이는가?
```

이렇게 보면 "사이트가 안 열린다"를 하나의 문제로 보지 않고, DNS, 로드밸런서, 방화벽, 프록시, 컨테이너, 애플리케이션 문제로 나눠서 추적할 수 있다.

## 최종 정리

```text
Nginx는 리버스 프록시로 active 컨테이너에 요청을 넘긴다.
Docker는 컨테이너 내부 포트와 호스트 포트를 분리해서 blue/green 구조를 가능하게 한다.
AWS VPC, Security Group, ALB는 서버를 더 안전하고 확장 가능한 구조로 운영하게 한다.
DNS와 HTTPS는 운영 서비스의 외부 진입점이다.
CI/CD와 모니터링은 배포와 장애 대응을 반복 가능하게 만든다.
```
