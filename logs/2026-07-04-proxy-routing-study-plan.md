...
총 공부 시간은 2시간 30분에서 3시간 정도로 잡는다.

```text
1교시: Nginx location 매칭 규칙 - 35분
2교시: proxy_pass 경로 처리 - 35분
3교시: X-Forwarded-* 헤더 - 30분
4교시: 502 / 503 / 504 장애 원인 - 35분
5교시: graceful shutdown과 블루/그린 배포 - 40분
마무리: 오늘 배운 내용 10줄 요약 - 15분
```

너무 많이 외우는 것보다, 각 주제마다 다음 질문에 답할 수 있으면 된다.
...
가능하면 실제 서버가 아니어도 로컬에서 흐름을 종이에 그려본다.

### 과제 1. Nginx location 결과 예측

아래 설정에서 각 요청이 어느 location으로 가는지 적어본다.
...
### 과제 2. proxy_pass 경로 예측

아래 두 설정에서 `/api/users` 요청이 백엔드에 어떤 path로 전달되는지 적어본다.
...
아래 상황을 502, 503, 504 중 어디에 가까운지 분류한다.
...
단일 EC2 blue/green 배포 순서를 직접 적어본다.
